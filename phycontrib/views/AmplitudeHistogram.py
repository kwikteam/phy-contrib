                ...
                for i in range(len(clusters)):
                    ...
                    if 0:
                        ...
                        try:
                            ...
                            try:
                                print("Fitting failed with maxfev=10000, "
                    ...
                    if was_fit[i]:
                        ...
                        # norm area calculated by summing
                        #norm_area = n_fit.sum() / popt[0] / popt[2]
                        #norm_area = norm_area / np.sqrt(2 * np.pi)
                        #norm_area = norm_area * np.diff(bins[:2])[0]
                        #percent_missing = 100 * (1 - norm_area)
                        #print(('Cluster {:d} is missing {:.1f}%' +
                        #      '(ntdr {:.1f}%) of spikes')
                        #      .format(clusters[i], percent_missing,
                        #              percent_missing_ndtr))
                        print('Cluster {:d} is missing {:.1f}%  of spikes'
                              .format(clusters[i], percent_missing_ndtr))
